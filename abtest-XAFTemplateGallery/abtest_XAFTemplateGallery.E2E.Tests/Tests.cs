...
namespace abtest_XAFTemplateGallery.Module.E2E.Tests;

public class abtest_XAFTemplateGalleryTests : IDisposable {
    const string BlazorAppName = "abtest_XAFTemplateGalleryBlazor";
    const string AppDBName = "abtest_XAFTemplateGallery";
    EasyTestFixtureContext FixtureContext { get; } = new EasyTestFixtureContext();

    public abtest_XAFTemplateGalleryTests() {
        FixtureContext.RegisterApplications(
            new BlazorApplicationOptions(BlazorAppName, string.Format(@"{0}\..\..\..\..\abtest_XAFTemplateGallery.Blazor.Server", Environment.CurrentDirectory))
        );
        FixtureContext.RegisterDatabases(new DatabaseOptions(AppDBName, "abtest_XAFTemplateGalleryEasyTest", server: @"(localdb)\mssqllocaldb"));
    }
    public void Dispose() {
